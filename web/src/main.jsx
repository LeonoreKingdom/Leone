...
function Chatbot() {
  ...
  const [form, setForm] = useState({ enabled: false, channelIds: [], triggerMode: 'mention_dm', retentionDays: 30, perUserCooldownSeconds: 8, dailyRequestLimit: 300, model: '' });
  const [notice, setNotice] = useState('');
  useEffect(() => {
    if (!state.data) return;
    const settings = state.data[0].settings;
    setForm({ enabled: Boolean(settings.enabled), channelIds: settings.channelIds ?? [], triggerMode: settings.triggerMode ?? 'mention_dm', retentionDays: settings.retentionDays ?? 30, perUserCooldownSeconds: settings.perUserCooldownSeconds ?? 8, dailyRequestLimit: settings.dailyRequestLimit ?? 300, model: settings.model ?? '' });
  }, [state.data]);
  if (state.loading || state.error) return <Status state={state} />;
