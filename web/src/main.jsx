...
function Chatbot() {
  ...
  const [form, setForm] = useState({ enabled: false, channelIds: [], triggerMode: 'mention_dm', retentionDays: 30, perUserCooldownSeconds: 15, dailyRequestLimit: 100, model: '' });
  const [notice, setNotice] = useState('');
  useEffect(() => {
    if (!state.data) return;
    const settings = state.data[0].settings;
    setForm({ enabled: Boolean(settings.enabled), channelIds: settings.channelIds ?? [], triggerMode: settings.triggerMode ?? 'mention_dm', retentionDays: settings.retentionDays ?? 30, perUserCooldownSeconds: settings.perUserCooldownSeconds ?? 15, dailyRequestLimit: settings.dailyRequestLimit ?? 100, model: settings.model ?? '' });
  }, [state.data]);
  if (state.loading || state.error) return <Status state={state} />;
