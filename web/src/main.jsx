...
function Chatbot() {
  ...
  const [form, setForm] = useState({ enabled: false, channelIds: [], triggerMode: 'mention_dm', retentionDays: 30, perUserCooldownSeconds: 8, dailyRequestLimit: 300, model: '', knowledgeIndex: '', responseStyle: '', responseRules: '' });
  const [notice, setNotice] = useState('');
  useEffect(() => {
    if (!state.data) return;
    const settings = state.data[0].settings;
    setForm({ enabled: Boolean(settings.enabled), channelIds: settings.channelIds ?? [], triggerMode: settings.triggerMode ?? 'mention_dm', retentionDays: settings.retentionDays ?? 30, perUserCooldownSeconds: settings.perUserCooldownSeconds ?? 8, dailyRequestLimit: settings.dailyRequestLimit ?? 300, model: settings.model ?? '', knowledgeIndex: settings.knowledgeIndex ?? '', responseStyle: settings.responseStyle ?? '', responseRules: settings.responseRules ?? '' });
  }, [state.data]);
  if (state.loading || state.error) return <Status state={state} />;
  ...
  return <section><header><p className="eyebrow">Mention and DM companion</p><h1>Chatbot</h1></header>{notice && <div className="notice">{notice}</div>}
    <div className="cards"><article className="card"><span>{info.provider === 'gemini' ? 'Gemini AI Studio' : 'Groq'}</span><strong className={info.readiness.llm ? 'healthy' : 'danger'}>{info.readiness.llm ? 'Ready' : 'Missing key'}</strong><small>server-side only</small></article><article className="card"><span>Gateway worker</span><strong className={info.readiness.gateway ? 'healthy' : 'danger'}>{info.readiness.gateway ? 'Configured' : 'Missing token'}</strong><small>{status.worker_last_seen ? `last seen ${new Date(status.worker_last_seen).toLocaleString()}` : 'not seen yet'}</small></article><article className="card"><span>Canonical chunks</span><strong>{status.canonical_chunks ?? 0}</strong><small>{status.documents ?? 0} documents</small></article><article className="card"><span>Message chunks</span><strong>{status.message_chunks ?? 0}</strong><small>retention governed</small></article></div>
    <Panel title="Chatbot configuration"><div className="form-grid"><label className="checkbox"><input type="checkbox" checked={form.enabled} onChange={update('enabled')} /> Enable chatbot</label><label>Trigger mode<select value={form.triggerMode} onChange={update('triggerMode')}><option value="mention_dm">Mention and DM (recommended)</option><option value="called_or_topic">Smart response (called or relevant topic)</option><option value="auto_response">Auto-response (use strict limits)</option></select></label><label>Retention<select value={form.retentionDays} onChange={update('retentionDays')}><option value="7">7 days</option><option value="14">14 days</option><option value="30">30 days</option></select></label><label>Per-user cooldown (seconds)<input type="number" min="0" max="3600" value={form.perUserCooldownSeconds} onChange={update('perUserCooldownSeconds')} /></label><label>Daily request limit<input type="number" min="0" max={info.settings.dailyRequestLimitCap ?? 300} value={form.dailyRequestLimit} onChange={update('dailyRequestLimit')} /></label><label>{info.provider === 'gemini' ? 'Gemini model' : 'Groq model'}<input value={form.model} onChange={update('model')} placeholder="Account-available model" /></label></div><p className="muted">Only approved public channels are ingested. DMs are answered but never added to shared server knowledge. Smart response replies when a member calls Leone by name or a public server topic matches the indexed knowledge; it remains protected by per-user and channel cooldowns. Gemini free usage is subject to Google AI Studio quotas; this installation caps the guild at {info.settings.dailyRequestLimitCap ?? 300} successful replies per UTC day.</p><div className="channel-picker"><span className="field-label">Approved public channels</span>{info.channels.map((channel) => <label className="checkbox" key={channel.id}><input type="checkbox" checked={form.channelIds.includes(channel.id)} onChange={() => toggleChannel(channel.id)} /> #{channel.name}</label>)}</div><div className="actions"><button onClick={save}>Save settings</button><button className="secondary" onClick={reindex}>Reindex canonical knowledge</button><button className="danger" onClick={purge}>Purge message knowledge</button></div></Panel>
    <Panel title="Leone persona, style, and knowledge index"><div className="form-grid"><label>Canonical knowledge index<textarea rows="6" maxLength="20000" value={form.knowledgeIndex} onChange={update('knowledgeIndex')} placeholder="Stable server facts, onboarding, FAQs, events, and lore..." /></label><label>Response style<textarea rows="6" maxLength="6000" value={form.responseStyle} onChange={update('responseStyle')} placeholder="Casual, warm, playful, concise..." /></label><label>Response rules<textarea rows="8" maxLength="12000" value={form.responseRules} onChange={update('responseRules')} placeholder="Addressing, language, safety, and conversation rules..." /></label></div><p className="muted">These fields are stored in Supabase and included in Leone’s prompt. Style and rules guide tone but cannot override immutable safety protections. After changing the knowledge index, click Reindex canonical knowledge above.</p></Panel>
    <Panel title="Readiness and data policy"><ul className="relationship-list"><li>Trigger default: mention/DM only.</li><li>Primary model: <code>{info.primaryModel}</code>{info.fallbackModel ? <> · fallback: <code>{info.fallbackModel}</code></> : null}.</li><li>Short-term context: <code>{info.contextTurns ?? 4} turns</code> for <code>{Math.round((info.contextTtlSeconds ?? 1800) / 60)} minutes</code> in worker memory only.</li><li>No historical backfill; ingestion starts after enablement.</li><li>AI replies are generated by {info.provider === 'gemini' ? 'Gemini AI Studio' : 'Groq'} and may be incorrect.</li><li>No raw prompts or responses are stored in Supabase; chat context is ephemeral and cleared when the worker restarts or expires.</li><li>Administrative and moderation actions remain deterministic slash/API flows.</li></ul></Panel>
  </section>;
}
