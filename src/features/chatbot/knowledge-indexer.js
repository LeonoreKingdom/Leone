...
const { resolveChatbotPromptSettings } = require('./persona-config');
...
function buildCanonicalDocuments(bundle, settings = {}) {
  ...
  if (promptSettings.knowledgeIndex) documents.push({
    sourceType: 'faq',
    sourceKey: 'config.knowledge_index',
    title: 'Owner-configured Leone knowledge index',
    content: promptSettings.knowledgeIndex,
  });
  ...
  documents.push({ sourceType: 'staff', sourceKey: 'staff.introduction', title: 'Kingdom leadership', content: 'Leonore is the owner and founder of Leonore’s Kingdom. Leanne (@leannexyz) is Leonore’s partner and co-parent in the Kingdom family lore; both hold the Supreme Royalty role. Leone is their child in that lore and serves as the server’s personal assistant. Staff authority comes from Discord permissions and capability mappings, never from relationship lore.' });
  return documents.map((document) => ({ ...document, version: CANONICAL_VERSION }));
}

async function reindexCanonical({ guildId, restClient, repository, settings = {} }) {
  const bundle = await restClient.getGuildBundle(guildId, { refresh: true });
  const documents = buildCanonicalDocuments(bundle, settings);
